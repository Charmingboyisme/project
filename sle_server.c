...
#define UUID_LEN_2 2
// 在文件顶部添加
extern void update_toilet_status_extended(uint8_t id, uint8_t state, uint8_t smoke, uint8_t paper);
...
// 添加外部声明
extern void update_toilet_status(uint8_t id, uint8_t state);
extern void update_toilet_status_with_extended_info(uint8_t id, uint8_t state, uint8_t smoke, 
                                   ...
                                   errcode_t status)
{
    unused(status);
    unused(server_id);
    printf("[%s] conn_id:%x, handle:%x, len:%d\n", 
          __FUNCTION__, conn_id, write_cb_para->handle, write_cb_para->length);
    
    // 检查是否为5字节扩展厕位状态消息 (ID + State + Entry_side + Paper + Msg_type)
    if (write_cb_para->length == 5 && write_cb_para->value != NULL) {
        uint8_t toilet_id = write_cb_para->value[0];
        uint8_t state = write_cb_para->value[1];
        uint8_t entry_side = write_cb_para->value[2];  // 新增：进入侧
        uint8_t paper = write_cb_para->value[3];
        uint8_t msg_type = write_cb_para->value[4];    // 新增：消息类型
        
        printf("Received extended status: Toilet %d -> State:%s Entry:%d Paper:%s Type:%d\n", 
              toilet_id, state ? "USED" : "FREE", entry_side,
              paper ? "LOW" : "OK", msg_type);
        
        // 调用扩展更新函数（需要新增或修改）
        update_toilet_status_with_extended_info(toilet_id, state, 0, paper, entry_side, msg_type);
    }
    // 检查是否为4字节厕位状态消息 (ID + State + Smoke + Paper)
    else if (write_cb_para->length == 4 && write_cb_para->value != NULL) {
        uint8_t toilet_id = write_cb_para->value[0];
        uint8_t state = write_cb_para->value[1];
        uint8_t smoke = write_cb_para->value[2];
        uint8_t paper = write_cb_para->value[3];
        
        printf("Received status: Toilet %d -> State:%s Smoke:%s Paper:%s\n", 
              toilet_id, state ? "USED" : "FREE",
              smoke ? "YES" : "NO", paper ? "OK" : "LOW");
        
        update_toilet_status_extended(toilet_id, state, smoke, paper);
    }
    // 兼容原有2字节格式
    else if (write_cb_para->length == 2 && write_cb_para->value != NULL) {
        uint8_t toilet_id = write_cb_para->value[0];
        uint8_t state = write_cb_para->value[1];
        
        printf("Received status: Toilet %d -> %s\n", 
              toilet_id, state ? "USED" : "FREE");
        
        update_toilet_status(toilet_id, state);
    }
    else if (write_cb_para->length > 0) {
...
// 向指定客户端发送命令
int sle_server_send_command_to_client(uint8_t toilet_id, uint8_t cmd_type, uint8_t param)
{
    if (g_sle_conn_hdl == 0) {
    ...
    uint8_t *cmd_data = osal_vmalloc(3);
    if (cmd_data == NULL) {
        return -1;
    }
    
    cmd_data[0] = toilet_id;
    cmd_data[1] = cmd_type;
    cmd_data[2] = param;
    
    msg_data.value = cmd_data;
    msg_data.value_len = 3;
    
    int ret = sle_server_send_report_by_handle(msg_data);
    osal_vfree(cmd_data);
    
    printf("Command sent to client: toilet_id=%d, cmd_type=%d, param=%d\n", 
...
/* Run the sle_entry. */
// app_run(sle_server_entry); // 注释掉这一行
#endif
